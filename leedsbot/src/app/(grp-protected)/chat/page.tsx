...
export default function ChatPage() {
  ...
  const [level, setLevel] = useState<Level>('BEGINNER'); // used for uploads
  ...
  // On load, start intake (doc-based questions come from API if docs exist)
  useEffect(() => {
    beginIntake();
  ...
  const onFilesChosen: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Re-run intake to regenerate doc-based questions
        await beginIntake();
      } else {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <p className="text-xs text-gray-500">
        Notes are pooled by subject + level for this prototype. Q/A are stored to improve future guidance & quizzes.
      </p>
    </div>
  );
}
